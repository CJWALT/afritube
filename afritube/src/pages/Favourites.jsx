            ...
            <div className="flex flex-col items-center  my-4" key={video.id}>
              ...
              <div className="bg-white w-[100%] px-6 py-4 rounded-b-[2rem] md:w-72">
                ...
                <div className="flex flex-row justify-between mt-4">
                  <Rating />
                 {favourites.find((vid) => vid.id === video.id) ? <MdFavorite className="text-2xl text-[red] cursor-pointer" onClick={()=>removeFromFav(video)}/> : <AiOutlineHeart className="text-2xl cursor-pointer" onClick={()=>addToFav(video)}/>}
                </div>
              </div>
            </div>
          ))}
